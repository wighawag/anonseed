...
- **Install / supervise SearXNG from the seed.** Out of scope by the spec: SearXNG's install method changes upstream (anonseed would chase it), and the per-account service is anonctl/anoncore's lifecycle. anonseed DETECTS + guides + writes the config only.
...
- **anonbox target here.** Not this task: on anonbox, webveil rides the staged IMAGE (SearXNG baked in, already running) per spec story 22b; the anonbox applier records that intent (`internal/anonbox`). This task is the anonctl socket path (story 22c).
